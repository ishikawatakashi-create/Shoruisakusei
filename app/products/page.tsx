    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="品目名で検索..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {products.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">品目がありません</TableCell>
              </TableRow>
            ) : (
              products.map((p) => (
                <TableRow key={p.id}>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <Label>単位</Label>
                <Input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="個,式,h" />
              </div>
              <div className="space-y-1.5">
                <Label>税率(%)</Label>
                <Select value={String(form.taxRate)} onValueChange={(v) => setForm({ ...form, taxRate: Number(v) })}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>デフォルト数量</Label>
                <Input type="number" min={0} step={0.01} value={form.defaultQuantity} onChange={(e) => setForm({ ...form, defaultQuantity: Number(e.target.value) })} />
              </div>
              <div className="space-y-1.5">
                <Label>並び順</Label>
